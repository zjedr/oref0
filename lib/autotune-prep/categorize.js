...
function categorizeBGDatums(opts) {
    ...
    //console.error(BGTime, lastBGTime, elapsedMinutes);
        ...
        //console.error(bucketedData[bucketedData.length-1].dateString, bucketedData[bucketedData.length-1].glucose, elapsedMinutes);
    }
    var interpolatedData = [];
    interpolatedData[0] = bucketedData[0];
    j=0;
    for (var i=1; i < bucketedData.length; i++) {
        if (bucketedData[i].dateString) {
            BGTime = new Date(bucketedData[i].dateString);
        }
        if (bucketedData[i-1].dateString) {
            lastBGTime = new Date(bucketedData[i-1].dateString);
        }
        var elapsedMinutes = (BGTime - lastBGTime)/(60*1000);
        //var glucoseDatum = bucketedData[i];
        //console.error(glucoseDatum.dateString, glucoseDatum.glucose);
        if(Math.abs(elapsedMinutes) > 8) {
            // interpolate missing data points
            lastbg = bucketedData[i-1].glucose;
            elapsedMinutes = Math.abs(elapsedMinutes);
            //console.error(elapsedMinutes);
            while(Math.abs(elapsedMinutes) > 2) {
                previousbgTime = new Date(lastBGTime.getTime() - 5 * 60*1000);
                j++;
                interpolatedData[j] = [];
                interpolatedData[j].date = previousbgTime.getTime();
                interpolatedData[j].dateString = previousbgTime.toISOString();
                gapDelta = bucketedData[i].glucose - lastbg;
                //console.error(gapDelta, lastbg, elapsedMinutes);
                previousbg = lastbg + (5/elapsedMinutes * gapDelta);
                interpolatedData[j].glucose = Math.round(previousbg);
                console.error("Interpolated", interpolatedData[j]);

                elapsedMinutes = elapsedMinutes - 5;
                lastbg = previousbg;
                lastBGTime = new Date(previousbgTime);
            }
        } else if(Math.abs(elapsedMinutes) > 2) {
            j++;
            interpolatedData[j]=bucketedData[i];
            interpolatedData[j].date = BGTime.getTime();
        }
    }
    bucketedData = interpolatedData;
